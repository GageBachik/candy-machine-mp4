...
programCommand('upload')
  ...
  .action(async (files: string[], options, cmd) => {
    ...
    log.info(`Beginning the upload for ${elemCount} (png+mp4+json) bundles`);
    ...
    if (!name.match(cacheItem.name) || !uri.match(cacheItem.link)) {
      ...
      if (json.status == 200 || json.status == 204 || json.status == 202) {
        ...
        if (parsed.image) {
          ...
          console.log('Name', name, 'with', uri, 'lacked image in json, failing');
          cacheItem.onChain = false;
          allGood = false;
        }
      } else {
        log.debug('Name', name, 'with', uri, 'returned no json from link');
        console.log('Name', name, 'with', uri, 'returned no json from link');
        cacheItem.onChain = false;
        allGood = false;
      }
    }
